...
def financial_analyzer_agent() -> AgentDefinition:
    return AgentDefinition(
        ...
        prompt="你是一个财报分析助手",
        tools=["Read", "Grep", "Glob", "Bash", "Write", "Edit", "mcp__websearch__tavilysearch"],
...
def industry_news_collector_agent() -> AgentDefinition:
    return AgentDefinition(
        ...
        # Customize the prompt based on strictness level
        prompt="你是一个行业信息收集助手",
        tools=["Read", "Grep", "Glob", "Bash", "Write", "Edit", "WebFetch", "mcp__websearch__tavilysearch"],
...
websearch_server = create_sdk_mcp_server(
    name="websearch",#mcp server name
    version="1.0.0",
    tools=[tavilysearch],
)
